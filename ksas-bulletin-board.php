 ...
 * Description: Creates a custom post type and widget for bulletins. Posts expire 9 months after publish date.
 * Version: 4.0
...
/** Define Categories for Bulletin Board */
...
class Bulletin_Board_Widget extends WP_Widget {
	...
	public function __construct() {
		$widget_options  = array(
			'classname'   => 'ksas_bulletin',
			'description' => __( 'Displays bulletin board entries based on category', 'ksas_bulletin' ),
		);
		$control_options = array(
			'width'   => 300,
			'height'  => 350,
			'id_base' => 'ksas_bulletin-widget',
		);
	...
	public function form( $instance ) {
		...
		$defaults = array(
			'title'           => __( 'Bulletin Board', 'ksas_bulletin' ),
			'quantity'        => __( '3', 'ksas_bulletin' ),
			'category_choice' => '1',
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php
		if ( $bulletin_board_query->have_posts() ) :
			while ( $bulletin_board_query->have_posts() ) :
				$bulletin_board_query->the_post();
				?>
		...
		<article aria-label="Bulletins Archive">
			<p><a href="
			<?php
			echo home_url( '/bbtype/' );
			echo $category_choice;
			?>
			">View more Bulletins <span class="fa fa-chevron-circle-right" aria-hidden="true"></span></a></p>
		</article>
...
function bulletin_plugin_activation() {
	if ( ! wp_next_scheduled( 'bulletin_alerts_cron' ) ) {
		wp_schedule_event( time(), 'daily', 'bulletin_alerts_cron' );
	}
}

/** Loop through all published posts and trash >=9 months old */
add_action( 'bulletin_alerts_cron', 'expire_bulletin_alert' );
function expire_bulletin_alert() {
	$args = array(
		'post_type'   => 'bulletinboard',
		'post_status' => 'publish',
	);
	...
	if ( $expire_query->have_posts() ) {
		...
		while ( $expire_query->have_posts() ) :
			$expire_query->the_post();

			$publish_time = get_the_time( 'U' ); // Returns our $publish time as a Unix timestamp.
			$delete_time  = $publish_time + 23328000; // 9 months = 23,328,000 sec.
...
register_deactivation_hook( __FILE__, 'bulletin_plugin_deactivation' );
/** Remove scheduled cron when plugin deactivated */
function bulletin_plugin_deactivation() {
	wp_clear_scheduled_hook( 'expire_bulletin_alert' );
}
